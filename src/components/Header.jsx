...
function Header() {
  ...
  return (
    ...
    <div className="fixed top-0 left-0 right-0 bg-white w-full h-[85px] flex justify-between items-center border-b border-b-gray-200 px-4 z-10">
      <img src={DashboardLogo} alt="Logo" className="h-[54px] object-contain" />
      ...
      <button onClick={() => navigate("/dashboard")} className="cursor-pointer flex items-center">
        <img src={Homepage} alt="Home" className="w-5 h-5" />
        <p className="hidden md:block md:ml-2.5">Trang chủ</p>
      </button>

      <button onClick={() => navigate("/notifications")} className="cursor-pointer flex items-center">
      ...
      <div className="mr-4 md:flex md:items-center md:gap-2 md:mr-12 relative">
        <div>
          ...
          <h6 className="text-sm opacity-50 leading-none">nguyenvana@gmail.com</h6>
        </div>
        <button onClick={() => setDropdownOpen((prev) => !prev)} className="cursor-pointer p-2">
        ...
        {dropdownOpen && (
          <div className="absolute right-0 top-[60px] bg-white shadow-lg rounded-lg w-48 mt-2 z-50">
            <ul>
              <li><button className="w-full px-4 py-2 hover:bg-gray-100 cursor-pointer"
                  onClick={handlePasswordChange}
                    >Đổi mật khẩu</button></li>
               <li><button onClick={() => navigate("/login")} className=" w-full px-4 py-2 hover:bg-gray-100 cursor-pointer">Đăng xuất</button></li>
            </ul>
          </div>
        )}
      </div>
